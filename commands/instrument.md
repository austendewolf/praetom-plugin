...
5. **Tell the user about the install step.** The runtime SDK is a one-time PR they open against their repo:
   ...
   registerBrowser({ ingestToken: "praetom_..." });
   ```
   Plus one env var on their deploy: `PRAETOM_INGEST_TOKEN=praetom_...`. Direct them to `/dashboard/integrations` to issue the token.
